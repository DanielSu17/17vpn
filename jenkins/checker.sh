...
config_env=$(git log  --name-only --pretty=format: "${GIT_PREVIOUS_SUCCESSFUL_COMMIT}".."${GIT_COMMIT}" | awk -F "/" '{print $2}' | sort -u)
...
main(){
    ...
    for env in ${config_env}; do
      ...
      docker run --rm -v "$(pwd)":/repo/configs 17media/config-checker:${env} "-check_configs=${env_files}"
    done
    ## get output status ##
